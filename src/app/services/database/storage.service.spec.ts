...
describe('StorageService', () => {
    ...
    describe('logReturns', () => {
        it('should log multiple returns and update data', async () => {
            // Arrange
            spyOn(service, 'loadData').and.returnValue(Promise.resolve());
            dbConnectionSpy.run.and.returnValue(Promise.resolve({ changes: { changes: 1 } } as capSQLiteChanges));
            dbConnectionSpy.execute.and.returnValue(Promise.resolve({ changes: { changes: 0 } } as capSQLiteChanges));
            const items = [
                { itemNo: 1, orderNo: 100, returnsNo: 5, generalNote: 'Return note' },
                { itemNo: 2, orderNo: 100, returnsNo: 3, generalNote: null }
            ];

            // Act
            await service.logReturns(items);
        ...
        it('should rollback and show toast on error', async () => {
            // Arrange
            dbConnectionSpy.run.and.returnValue(Promise.reject(new Error('DB error')));
            dbConnectionSpy.execute.and.returnValue(Promise.resolve({ changes: { changes: 0 } } as capSQLiteChanges));
            const items = [{ itemNo: 1, orderNo: 100, returnsNo: 5, generalNote: 'Return note' }];

            // Act
            await service.logReturns(items);

            // Assert
            expect(dbConnectionSpy.execute).toHaveBeenCalledWith('ROLLBACK');
    ...
    describe('logDiscrepency', () => {
        it('should log discrepancy and update data when item and invoice exist', async () => {
            // Arrange
            spyOn(service, 'getSingleInvoiceItem').and.returnValue(Promise.resolve([mockInvoiceItem]));
            spyOn(service, 'getInvoicebyOrderNo').and.returnValue(Promise.resolve([mockInvoice]));
            spyOn(service, 'loadData').and.returnValue(Promise.resolve());
            ...
            // Act
            await service.logDiscrepency(1, 100, 2, 'Discrepancy note');

            // Assert
            expect(dbConnectionSpy.run).toHaveBeenCalledWith('UPDATE invoiceitems SET discrepancies = ? WHERE orderNo = ?', [2, 100]);
            expect(dbConnectionSpy.run).toHaveBeenCalledWith('UPDATE invoices SET generalNote = ? WHERE orderNo = ?', ['Discrepancy note', 100]);
            expect(service['loadData']).toHaveBeenCalled();
            expect(toastSpy).not.toHaveBeenCalled();
        });
